...
export default function MovieStickyHeader({
  ...
  useEffect(() => {
    const handler = () => {
      // Скролл 150-аас их болсон үед Header багасна
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-300 ease-in-out px-4 sm:px-10 lg:px-20 flex flex-row justify-between items-center bg-white dark:bg-[#0a0a0a] border-b dark:border-gray-800 ${
        isShrunk 
          ? "py-2 h-16 shadow-lg bg-opacity-90 backdrop-blur-md" 
          : "py-6 h-32"
      }`}
    >
      <div className="flex flex-col flex-1 min-w-0">
        ...
        {/* Багассан үед жижиг мэдээллүүдийг өндрийг нь 0 болгож нууна */}
        <div 
          className={`flex items-center font-medium gap-2 text-gray-500 overflow-hidden transition-all duration-300 ${
            isShrunk ? "h-0 opacity-0 mt-0" : "h-6 opacity-100 mt-1 text-[10px] sm:text-sm"
          }`}
        >
          <p>{release_date}</p>
          <span>•</span>
      ...
      <div className="flex items-center gap-2 shrink-0 ml-4">
        ...
        <div className="flex flex-col items-end">
          <p className={`font-bold dark:text-white transition-all duration-300 ${
            isShrunk ? "text-sm" : "text-base sm:text-lg"
          }`}>
            {vote_average?.toFixed(1)}
            <span className="text-gray-400 font-normal text-xs ml-0.5">/10</span>
          </p>
          {!isShrunk && (
            <p className="text-[10px] text-gray-400 animate-in fade-in duration-500">
